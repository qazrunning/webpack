...
// 外检图图片上传地址
export const getexternalImgIP = async () => {
  // let IP = "";
  let url = "api/hj/getexternalImgIP";
  await app.curl
    .get(url)
    .then((res) => {
      // if(res.state) IP = res.IP;
      if (res.state) {
        config.externalImgIP.LLAN = res.IP;
        config.externalImgIP.WLAN = res.IP;
      }
    })
...
// 冒黑烟车抓拍视频图像文件路径
export const getSmokeCapturePath = async () => {
  let url = "api/hj/getSmokeCapturePath";
  await app.curl
    .get(url)
    .then((res) => {
      if (res.state) config.smokeUrl = res.IP;
    })
    .catch((error) => console.log(error));
};
// 检测图片是否读取中心端
export const getisImageCenterEnd = async () => {
  let url = "api/hj/getisImageCenterEnd";
  await app.curl
    .get(url)
    .then((res) => {
      if (res.state) config.isImageCenterEnd = res.data;
    })
    .catch((error) => console.log(error));
};
// 检测视频是否读取中心端
export const getisVideoCenterEnd = async () => {
  let url = "api/hj/getisVideoCenterEnd";
  await app.curl
    .get(url)
    .then((res) => {
      if (res.state) config.isVideoCenterEnd = res.data;
    })
    .catch((error) => console.log(error));
};
// 审核备注是否能为空
export const getreSure = async () => {
  let url = "api/hj/getreSure";
  await app.curl
    .get(url)
    .then((res) => {
      if (res.state) config.reSure = res.data;
    })
    .catch((error) => console.log(error));
};
getexternalImgIP();
