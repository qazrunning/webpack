...
<template>
  <div>
    ...
    <Button v-show="isSnap" style="margin-left:10px;" size="small" shape="circle" type="primary"
...
export default {
  ...
  methods: {
    ...
    async saveAudit(isStauts) {
      ...
      const res = await this.$curl.post("api/hj/SaveChangeCheckDateAudit", param);
      if (res.state) {
        if (res.auditInfo) {
          //将后台返回的处理人，处理时间赋值
  ...
  watch: {
    selectData: {
      handler(newData, oldData) {
        if (!newData) return;
        if(newData.ApplyReason&&newData.ApplyReason.includes('05')) {
          this.isSnap = true;
        }else {
          this.isSnap = false;
        }
