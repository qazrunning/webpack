      ...
      <div class="" v-show="isShow" style="position:absolute;right:20px;top:50px;z-index:100;border-radius: 4px">
        <Card title="条件" icon="ios-options" :padding="15" shadow style="width: 350px;">
          <Form class="" :label-width="100">
            <FormItem class="item" label="受理日期：" style="margin: 5px 0px;width: 100%;">
              <DatePicker class="itemValue" type="daterange" :split-panels="true" placeholder="请选择受理日期" v-model="AcceptanceDate" style="width: 100%;"></DatePicker>
            </FormItem>
  ...
  methods: {
    ...
    handleSure() {
      const params = {
        ...
        AcceptanceDate: this.selectDate.AcceptanceDate,
        AcceptanceAuditID: this.selectDate.AcceptanceAuditID
      }
      this.$curl.post('api/hj/editMutiAcceptanceAuditHowLongTimes', params).then(res => {
        if (res.state) {
    ...
    //获取数据
    getDate() {
      this.searchInfo.AcceptanceDate1 = formatDates(this.AcceptanceDate[0], 'yyyy-MM-dd 00:00:00')
      this.searchInfo.AcceptanceDate2 = formatDates(this.AcceptanceDate[1], 'yyyy-MM-dd 23:59:59')
      const params = {
        currentPage: this.currentPage,
        pageSize: this.pageSize,
        ...this.searchInfo
      }
      this.$curl.get('api/hj/geMutiAcceptanceAuditDate', { param: JSON.stringify(params) }).then(res => {
        if (res.state) {
