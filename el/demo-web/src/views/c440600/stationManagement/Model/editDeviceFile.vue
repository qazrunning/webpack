...
export default {
  ...
  methods: {
    ...
    async searchFileInfo() {
      const self = this;
      const params = { StationDeviceID: this.StationDeviceID }
      let result = await this.$curl.get("api/hj/getDeviceimg", { StationDeviceID: this.StationDeviceID });
      if (result.state) {
        result.data.forEach(t => {
          t.StationCode = self.StationCode;
          t.StationDeviceID = self.StationDeviceID;
          if (t.ID) { //是否有图片
            t["HasPhoto"] = t.FullFileName ? 1 : 0;
