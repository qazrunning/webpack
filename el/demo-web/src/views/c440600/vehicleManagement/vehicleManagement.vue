  ...
  <div class="content" ref="content">
    ...
    <div class="main">
      ...
      <div class="main_left fx__bgcolor">
        <div class="item-box" style="display: flex">
          ...
          <Select v-model="conditions.searchVehicle.cpys" clearable placeholder="车牌颜色" style="flex: 1; margin: 6px">
            <Option v-for="(item, index) in selectDict.VLPNColor" :value="item.CodeValue" :key="index">{{
              item.CodeName
            }}</Option>
          </Select>
        </div>
        ...
        <div v-for="(item, index) in results.vehicleList" :key="index" @click="initVehicle(item)">
          <Card
            ...
            :class="{ 'fx__item-selected': item.VehicleID == ID }" 
          >
            <div class="card_title" >
              <div class="card_tit_left fx__simpleline">
                <span title="车牌号" class="fx__text_middle" :style="setVLPNColor(item.VLPNColor)">{{
                    item.VLPN || "无车牌"
                }}</span>
                <div class="car_tit_info">
                  ...
                  <div title="车辆类型" v-if="item.GAVType" class="info_item">
      ...
      <div class="main_right fx__box">
        ...
        <!-- isShowAcceptance -->
        <!-- <acceptanceModel :isShowAcceptance="status.isShowAcceptance" @changeShowAcceptance="changeShowAcceptance" :VehicleID="status.VehicleID"/> -->
      </div>
    </div>
  </div>
</template>
...
import {setVLPNColor} from "../../../utils/tools";
export default {
  //   name: "content",
  components: { acceptanceModel },
  data() {
    return {
      setVLPNColor,
      tables:{
  ...
  methods: {
    ...
    initSelectDict(){
      ...
      let yw=[{ CodeName: "有", CodeValue: "1" }, { CodeName: "无", CodeValue: "0" }];
      ...
      this.selectDict['HasFueltankCover']=yw;//是否有油箱盖
      this.selectDict["ssx"]=this.$XZQH;//行政区划
      this.selectDict['cpys']={
        '01':{'background':'#0000FF','color':'#FFFFFF'},
        '02':{'background':'#FFFF00','color':'#000000'},
        '03':{'background':'#000000','color':'#FFFFFF'},
        '04':{'background':'#FFFFFF','color':'#000000'},
        '05':{'background':'#00FF00','color':'#000000'},
        '06':{'background':'#8EC21F','color':'#000000'},
      }
      ...
      this.selectDict['fuelTypeMap']=this.$GY_MAP('cd_fueltype');
      this.selectDict['gavtypeMap']=this.$GY_MAP('cd_gavtype');
      // this.$set(this.selectDict,'VLPNColor',this.$GY_ARRAY("cd_vlpncolor"));
      // this.$forceUpdate();
    },
    //初始化车辆
    initVehicle(item){
        ...
        this.$curl.get('/api/common/query/searchVehicleDetailList',{VehicleID:item.VehicleID}).then(res=>{
            if(res.code==0){
              if(res.data.length>0){
                this.initVehicleInfo(res.data[0]);
              }
            }
        })
      }else{
    ...
    initData(){
      ...
      this.surplusItems = this.$options.data().surplusItems;
      this.conditions = this.$options.data().conditions;
    },
    //初始化车辆信息
    initVehicleInfo(newV){
      this.ID = newV.VehicleID;
      this.initData();
      if(newV!=null){
        for(let key in newV){
          if(this.vehicleItems[key]){
            ...
            this.surplusItems[key]=newV[key]==null?"":(newV[key]+"");
    ...
    showOrHidden(obj,name,key){
      const c=(rule, value, callback) => {
        //console.log(this[name][key].required,value,obj.describe)
        if ((value==null||value==="") && this[name][key].required) {
          return callback(new Error(`${obj.describe}不能为空`));
        } else {
          return callback();
        }
    ...
    async acceptance(){
      //判断是否已经存在受理信息
      this.$curl.get("/api/c440600/Inspection/Acceptance/CheckVehicleInAcceptanceRedis", {
        VehicleID:this.vehicleItems.VehicleID.value
      }).then((res) => {
        if(res.code==0){
          if(res.data){
            if (window.confirm('该车已经存在受理信息，是否继续受理？')) {
              this.status.VehicleID=this.vehicleItems.VehicleID.value;
              this.status.isShowAcceptance=true;
            }else{
              return;
            }
          }else{
    ...
    //改变
    changeShowAcceptance(){
      this.status.isShowAcceptance=false;
    },
    getSubmitData(){
      let res={};
      ...
      for(let key in this.surplusItems){
        if(!res.hasOwnProperty(key)){
          res[key]=this.surplusItems[key];
        }
      }
      delete res.ssx;
      return res;
    ...
    extValidate(params){
      ...
      if(params.VIN!=null&&params.VIN.length!=17){
        if (!window.confirm('输入的车架号不是17位，确定要保存吗？')) {
          return;
        }
        // this.$Modal.confirm({
        //     title: '车架号提示',
            ...
            if(res.code==0){
              ...
              .post(
                ...
                //   this.$emit("initVehicle",data.Vehicle);
                  this.initVehicleInfo(data.Vehicle);
