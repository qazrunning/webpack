...
<template>
    ...
    >
        ...
        <div slot="title" style="height: 22px" v-if="false">
            <RadioGroup
                style="margin: 4px 0"
                v-model="selVideoType"
                @on-change="RadioChange"
        ...
        <a
            ...
            :href="playBackHrefs"
            v-if="isReady && IsFinishCheck == 1"
            >回放视频
        </a>
        <a
            style="display: block; margin: 20px auto; text-align: center"
            :href="hrefs"
            v-if="isReady && IsFinishCheck == 0"
            >实时视频
        </a>
        <div
            v-if="false"
...
import { WebVideoCtrls } from "../../../../../public/hj/js/webVideoCtrl.js";
...
export default {
    ...
    computed: {
        hrefs() {
            ...
            szStartTime = this.InspectTimes.startTime;
            szEndTime = this.InspectTimes.endTime;
            ...
            return `alert:http://${host}/DH/index.html?selStation=${encodeURIComponent(
                selStation
            )}?
        ...
        playBackHrefs() {
            ...
            szStartTime = this.InspectTimes.startTime;
            szEndTime = this.InspectTimes.endTime;
            return `alert:http://${host}/DH1/index.html?selStation=${encodeURIComponent(
                selStation
            )}?szStartTime='${szStartTime}'?szEndTime='${szEndTime}'?IsDVRUseMapIP=${
                this.IsDVRUseMapIP
    ...
    methods: {
        ...
        async getDVRUseMapIPConfig() {
            let res = await this.$curl.get("api/hj/getAllSysConfig", { param: JSON.stringify({where: "IsDVRUseMapIP", sysConfigType: "02"})})
            ...
            this.$curl
                ...
                .then((res) => {
                    if (res.DVRInfo) {
                        ...
                        this.szMapIP = res.DVRInfo.MapIP; //映射IP
