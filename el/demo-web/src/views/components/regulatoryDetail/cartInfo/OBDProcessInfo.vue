...
<template>
  <Card v-resize="resize" :bordered="false" style="position:relative;">
    <Button
      ...
      type="primary"
      size="small"
      v-show="tagsName == 'table' && resData.length > 0"
      style="position:absolute;right:5px;top:5px;z-index:200"
    >
      <Icon type="ios-download-outline"></Icon>导出
    </Button>
    <Tabs size="small" v-model="tagsName" :animated="false">
      <TabPane label="OBD过程数据列表" name="table">
        <Table
          ref="table"
          :columns="columns"
          :data="resData"
      ...
      <TabPane label="OBD过程数据图表" name="charts">
        <div
...
export default {
  ...
  methods: {
    ...
    resize() {
      if (this.OBDProcess_div) this.OBDProcess_div.resize();
    },
    //数据过滤
    toFloatTwo(value) {
    ...
    Process_chart() {
      try {
        //  if (this.OBDProcess_div) {
        //   this.OBDProcess_div.clear();
        //   this.$echartsc.dispose(this.OBDProcess_div);
        //   this.OBDProcess_div=null;
        // }
        ...
        this.OBDProcess_div = this.$echartsc.init(this.$refs.OBDProcess_div);
        // 指定图表的配置项和数据
        var option = {
          color: [
            "#00BCD4",
        ...
        this.OBDProcess_div.setOption(option, true);
      } catch (error) {
        console.log(error);
      }
    },
